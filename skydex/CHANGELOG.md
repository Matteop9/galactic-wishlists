...
### Fixed — the facing cone pointing somewhere the phone isn't (field report, 27 Aug)
Report: "every now and then the app is pointing a different direction to the compass" — the map cone reading ~344° with iOS Compass on 243° in the same minute. Three independent ways `app/spot/page.tsx` could draw a confidently wrong heading, all of them silent:
- **A frozen heading survived a dead sensor stream.** The orientation listener attaches once and `heading` is only ever overwritten by a new event — so when iOS stops delivering `deviceorientation` (webview suspended on app background / long screen-off; WKWebView does not always restart CoreMotion on resume) the cone stayed pinned to wherever the phone last faced, indistinguishable from a live reading. New `HEADING_STALE_MS = 6 s` watchdog (1 s ticker over a `headingAtRef` stamp) clears `heading` rather than keep a stale one, and `reattachOrientation()` genuinely removes + re-adds the listener (WebKit starts/stops CoreMotion off the listener count, so re-requesting permission alone is a no-op). Same re-arm on `visibilitychange` → visible, which is when the stream is usually found dead.
- **iOS own accuracy figure was ignored.** `webkitCompassAccuracy` (degrees; negative = uncalibrated) is now read alongside the heading; past `HEADING_ACCURACY_TOL = 30` — an error bar wider than `CONE_TOL` itself — the cone becomes a hint: faded fill/line via a new `headingUncertain` prop on `components/SpotMap.tsx`, `⚠` in the camera HUD, and "compass unreliable — wave the phone in a figure-8" under the map.
- **Heading was device-frame, never screen-compensated.** Both `webkitCompassHeading` (CLHeading, portrait-referenced — WebKit never sets `headingOrientation`) and `360 − alpha` report the yaw of the phone top edge, which is where the camera looks only in portrait; rolled into landscape to frame a plane it reads ~90° off the aim. Now `+ screenAngle()` (`screen.orientation.angle`, `window.orientation` fallback) — the heading twin of the portrait-only pitch bug `cameraElevation` fixed. Portrait is unchanged (angle 0); **needs a device check in landscape** (aim at a landmark, rotate the phone, HDG must not move) to confirm the sign.
- **Diagnosability**: the HDG readout was camera-only. The map view now prints `HDG n° ±acc°` as well, so the next occurrence can be compared against the phone compass app from the screen where it shows.

### Changed — quieter viewfinder while you are actually aiming (field report, 27 Aug)
"The UI is a bit full when you are trying to catch planes." Everything on the camera preview was permanent: an HDG/ELV/in-range telemetry line, a full-sentence tracking banner (`Tracking G-XXXX — aim to confirm, or capture anyway`), a `reg · 12 km · 7° off` caption, a ghost glyph and a zoom slider parked across the bottom. The frame is now mostly sky, with the numbers one tap away.
- **`app/spot/page.tsx`**: telemetry collapses to a 24 px corner chip (tap to expand, remembered per device in `localStorage.skydex_hud_details` — new `HUD_KEY`, `details` state). Collapsed it still shows `⚠` when the compass is unreliable or dropped out, since that one is actionable.
- **Tracking / In-sights banners → one pill.** Two words, no registration (the reg is already on the target caption and on the capture button), moved from `top-12` up onto the same line as the chip: `Tracking` (sky) / `In sights` (stamp). The "aim to confirm, or capture anyway" instruction was already the capture button's own label.
- **Zoom slider auto-hides.** Shows for 2.5 s after any zoom change (`applyZoom` → `bumpZoomUi()`; pinch, wheel, slider drag, camera start) and otherwise leaves just a `2.4×` chip that taps to bring it back — nothing at all at 1×. Pinch was always the primary gesture.
- **`components/TargetOverlay.tsx`**: new `details` prop. Caption trims to `reg · N°` (the one number that changes what you do next); distance and the raw-fix ghost — both aim diagnostics — now ride with the details chip.

### Fixed — map overlays hidden behind the basemap copyright
- **`components/SpotMap.tsx`** + **`app/spot/page.tsx`**: MapLibre's compact attribution (`attributionControl: { compact: true }`) owns the bottom ~34 px of the map container, collapsed or expanded, and both the Recenter button and the sweep status chip ("Scanning the sky…" / "No aircraft in range right now") sat at `bottom-3` — 12 px — underneath it. Both lift to `bottom-10`; the attribution stays exactly where it is, since visible credit is a CARTO/OSM licence condition. The camera view's zoom chrome keeps `bottom-3` (no attribution control over the preview).
