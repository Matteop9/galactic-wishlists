...
- **`Milky Bay/`** — Weekly two-acca tracker for a second friend group (Harry, Luke, Tim, Sandy, Liam), forked from `JHH Acca/`. **Same Supabase project as The Acca**, isolated in the `milkybay` Postgres schema (migrations `mb_00xx`); PostgREST exposure is the in-database `pgrst.db_schemas` role setting — if you ever edit "Exposed schemas" in the dashboard, run `alter role authenticator reset pgrst.db_schemas` first. **Logins shared with The Acca** (same `@players.jhh-acca.app` alias domain; `link_player` RPC claims a Milky Bay name for an existing Acca login; unlink in either app guarded — JHH migration `0022`; password resets affect both apps). Rules: two picks/player/week (W acca min 1.50 + Random acca min 1.70 bet-builder, fractional odds accepted), win = odds capped 2.50, sole-loser penalty −1×odds uncapped, no-pick −1. Season named **26/27** (gameweeks match real PL 26/27 weekends). Mini leagues are first-class (admin creates + assigns GWs per row; "Jersey Weekend" = first 6 seeded). Rules are admin-editable in-app (`rules_sections`, audited). Honours seeded 22/23→25/26 (crowns / half-crown / spoons / half-spoon) + full 24/25 & 25/26 tables in `season_history`; never-won-a-season players wear a tiny 💩 over their name (half poo for a half-season-only winner). Feedback queue: submit on You, triage in Admin. No teams, no live scores, no LLM. Deadlines social (Thu 8pm chat); app window Wed noon → Sat midnight, pg_cron `mb-gw-tick` sweeps no-picks. Stack: Vite + React + TS + Tailwind v4; same env vars as JHH. Deploy: `vercel deploy --prod` from the subfolder.
- **`LeaguePredictor/`** — "Spot On": predict-the-final-table game with mates, live at spot-on-liart.vercel.app. Predict the complete final table + top scorer per competition before the season; 1 pt per position off, −5 for calling the top scorer, lowest wins. Leagues with invite codes; picks hidden until a lock deadline, then live leaderboard. Stack: Next.js 16 (App Router) + Vercel Blob **private** store (versioned-doc pattern, reads need `Authorization: Bearer BLOB_READ_WRITE_TOKEN`) + api.football-data.org (cached in-store: standings 15 min / scorers + fixtures 1 h / teams 7 d — free tier is 10 req/min, never call per page view; ⚠️ some competitions return junk datetime strings as match `status`, so fixture filtering deny-lists dead states + checks `score.winner`). Hand-rolled auth (bcryptjs + jose JWT cookie). Pure engines, both unit-tested: scoring `src/lib/scoring.ts` (golden-tested against the original Excel) + cheering logic `src/lib/rooting.ts`. Post-lock pages: the grid `/leagues/[id]/table` (tabs: everyone's picks board / vs the actual table) and `/leagues/[id]/fixtures` (who-to-cheer-for guide + "the room"). ⚠️ `Key.txt` + `Standings.xlsx` contain the API token and are gitignored — never commit them (public repo). Deploy: **push to main** — the Vercel project is git-connected with Root Directory `LeaguePredictor` and a `vercel.json` ignoreCommand that skips builds when a push doesn't touch this folder; `vercel deploy --prod` from the subfolder NO LONGER works with a Root Directory set. Authed local smoke test: `node scripts/smoke.mjs <leagueId>` against `next dev`.

- **`skydex/`** — "SkyDex": verified plane-spotting logbook, live at sky-dex.com + iOS App Store (bundle `com.skydex.mobile`). Photograph an aircraft you can actually see; server verifies GPS + compass + pitch against live ADS-B; the catch becomes a rarity-graded collectible card. Books, feed + comments, leaderboards, community photo review, user blocking. Stack: Next.js 16 (App Router) + Supabase (Postgres + RLS + Apple/Google OAuth, EU/London) + ADS-B providers (adsb.lol/adsb.fi/airplanes.live) + FR24 API + Capacitor 8 hosted-mode iOS shell (Codemagic CI at repo-root `codemagic.yaml`) + Vercel. Release runbook in `skydex/AGENTS.md` (manual `npx vercel --prod` deploys, migrations via Supabase MCP); App Store review pack in `skydex/APP_REVIEW.md`.
