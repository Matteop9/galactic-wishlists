...
/* Admin -> Team logos. Every club anyone has ever picked is checked against the
   ...
   listed here with a one-click TheSportsDB lookup, a paste-a-URL escape hatch,
   and a skip option for names that are not clubs at all ('Draw'). Saves land in
   team_badges (migration 0025), so a new crest is live for everyone the moment
   it is set -- no fetch-badges.ts rerun, no deploy. */
...
export default function AdminTeamLogos() {
  ...
  // Still on an initials chip: not a sport, no build-time crest, and no
  // decision recorded in team_badges yet.
  const missing = (dict ?? []).filter(
    (t) => !SPORTS.has(t.name) && !crestUrl(t.name) && !decided.has(t.name),
  )
      ...
      {!dictLoading && (
        <>
          ...
          <p className="mt-3 text-[10.5px] leading-snug text-muted">
            FIND searches TheSportsDB by name · URL accepts any https image link · SKIP marks a name
            as never needing a logo. Whatever you set shows up for everyone straight away.
          </p>
        </>
      )}
    </div>
