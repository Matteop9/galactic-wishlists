...
/* Searchable team picker for Enter Pick: type to filter every team ever
   used (with crest + usage count), or just keep typing for a brand-new one.
   Free text is always allowed — the dropdown is a shortcut, not a gate. */
        ...
        >
          {filtered.map((o) => (
            ...
            >
              ...
              <span className="font-mono text-[9px] text-muted">{o.uses}×</span>
            </button>
          ))}
          <div className="px-3 py-1.5 text-[9.5px] text-muted" style={{ borderTop: '1px solid var(--color-line)' }}>
            First time for a team? Just type the name in full.
          </div>
        </div>
      )}
    </div>
