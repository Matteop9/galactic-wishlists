...
- Trade history (v0.7.0): refresh pulls all completed trades for this season + `thresholds.trades.seasonsBack` previous seasons (via `previous_league_id`; note Sleeper returns "0" for none) into `snapshot.leagues[].trades` (raw, all managers, with per-season `ownerByRosterId` since roster ids change owners across seasons). `engine/tradeHistory.ts` reviews MY trades with today's values — already-drafted picks and off-market players are flagged untraceable, not silently zeroed. Live fetch skips trade history (~20 extra calls per league per season).
